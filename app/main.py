...
try:
    ...
    # Não derruba a aplicação: /readyz reporta indisponibilidade.
    logging.getLogger(__name__).warning("db_init_failed: %s", exc)
...
def livez():
    """Liveness: o processo responde sem checar dependências."""
    return {"status": "ok"}


@app.get("/readyz", tags=["health"])
def readyz(db: Session = Depends(get_db)):
    """Readiness: verifica se as dependências estão disponíveis."""
    try:
        db.execute(text("SELECT 1"))
    except Exception:
        raise HTTPException(
